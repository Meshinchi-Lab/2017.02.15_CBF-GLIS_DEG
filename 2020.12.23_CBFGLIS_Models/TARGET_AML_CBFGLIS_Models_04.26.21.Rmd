...
require(knitr)
PROJHOME  <- if(grepl("gizmo",  Sys.info()[["nodename"]])){
  file.path("/fh/fast/meshinchi_s/workingDir/TARGET/AML_TARGET/RNA/mRNAseq/analysis")
}else{
  file.path("~/fast_drive/workingDir/TARGET/AML_TARGET/RNA/mRNAseq/analysis")
}
...
gsva_DE <- function(gsva_matrix,clinData,col,p.value=0.001){
  ...
  allGeneSets <- topTable(fit2, p.value = p.value,number=Inf) %>% 
    ...
    #object 'logFC' not found ℹ Input `..1` is `logFC???? And this error only happens in a lapply()
